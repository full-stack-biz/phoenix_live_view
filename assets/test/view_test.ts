...
import LiveSocket from "../src/phoenix_live_view";
...
describe("View + DOM", function () {
  test("update", async () => {
    ...
    expect((view.el.firstChild as HTMLElement).tagName).toBe("H2");
  ...
  test("pushEvent as checkbox when checked", function () {
    ...
    let liveSocket = new LiveSocket("/live", Socket);
    let el = liveViewDOM();
    let input = el.querySelector(`input[type="checkbox"]`) as HTMLInputElement;
    input.checked = true;
...
describe("View + Component", function () {
  ...
  test("empty diff undoes refs and pending attributes", () => {
    ...
    expect(view.el.innerHTML).toBe(
      `<form phx-submit="submit" phx-page-loading="" class="phx-submit-loading" data-phx-ref="0"><input type="text" data-phx-readonly="false" readonly="" class="phx-submit-loading" data-phx-ref="0"></form>`
    );
...
describe("DOM", function () {
  it("mergeAttrs attributes", function () {
    const target = document.createElement("target") as HTMLInputElement;
